"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Organizations
...
logger = logging.getLogger("infraflowx.organizations.workflow")


class OrganizationsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling organizations entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for organizations #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for organizations #{entity_id}: {curr} -> {target} by {user_role}")
