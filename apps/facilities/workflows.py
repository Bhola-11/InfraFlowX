"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Facilities
...
logger = logging.getLogger("infraflowx.facilities.workflow")


class FacilitiesWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling facilities entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for facilities #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for facilities #{entity_id}: {curr} -> {target} by {user_role}")
