"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Workorders
...
logger = logging.getLogger("infraflowx.workorders.workflow")


class WorkordersWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling workorders entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for workorders #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for workorders #{entity_id}: {curr} -> {target} by {user_role}")
