"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Roads
...
logger = logging.getLogger("infraflowx.roads.workflow")


class RoadsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling roads entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for roads #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for roads #{entity_id}: {curr} -> {target} by {user_role}")
