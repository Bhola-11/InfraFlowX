"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Employees
...
logger = logging.getLogger("infraflowx.employees.workflow")


class EmployeesWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling employees entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for employees #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for employees #{entity_id}: {curr} -> {target} by {user_role}")
