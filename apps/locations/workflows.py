"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Locations
...
logger = logging.getLogger("infraflowx.locations.workflow")


class LocationsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling locations entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for locations #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for locations #{entity_id}: {curr} -> {target} by {user_role}")
