"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Buildings
...
logger = logging.getLogger("infraflowx.buildings.workflow")


class BuildingsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling buildings entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for buildings #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for buildings #{entity_id}: {curr} -> {target} by {user_role}")
