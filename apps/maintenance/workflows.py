"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Maintenance
...
logger = logging.getLogger("infraflowx.maintenance.workflow")


class MaintenanceWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling maintenance entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for maintenance #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for maintenance #{entity_id}: {curr} -> {target} by {user_role}")
