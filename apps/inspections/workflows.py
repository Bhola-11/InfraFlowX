"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Inspections
...
logger = logging.getLogger("infraflowx.inspections.workflow")


class InspectionsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling inspections entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for inspections #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for inspections #{entity_id}: {curr} -> {target} by {user_role}")
