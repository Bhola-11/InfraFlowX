"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Projects
...
logger = logging.getLogger("infraflowx.projects.workflow")


class ProjectsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling projects entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for projects #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for projects #{entity_id}: {curr} -> {target} by {user_role}")
