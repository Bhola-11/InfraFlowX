"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Notifications
...
logger = logging.getLogger("infraflowx.notifications.workflow")


class NotificationsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling notifications entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for notifications #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for notifications #{entity_id}: {curr} -> {target} by {user_role}")
