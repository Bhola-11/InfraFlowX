"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Analytics
...
logger = logging.getLogger("infraflowx.analytics.workflow")


class AnalyticsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling analytics entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for analytics #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for analytics #{entity_id}: {curr} -> {target} by {user_role}")
