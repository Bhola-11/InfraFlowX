"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Budgets
...
logger = logging.getLogger("infraflowx.budgets.workflow")


class BudgetsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling budgets entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for budgets #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for budgets #{entity_id}: {curr} -> {target} by {user_role}")
