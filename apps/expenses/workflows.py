"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Expenses
...
logger = logging.getLogger("infraflowx.expenses.workflow")


class ExpensesWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling expenses entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for expenses #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for expenses #{entity_id}: {curr} -> {target} by {user_role}")
