"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Documents
...
logger = logging.getLogger("infraflowx.documents.workflow")


class DocumentsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling documents entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for documents #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for documents #{entity_id}: {curr} -> {target} by {user_role}")
