"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Schedules
...
logger = logging.getLogger("infraflowx.schedules.workflow")


class SchedulesWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling schedules entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for schedules #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for schedules #{entity_id}: {curr} -> {target} by {user_role}")
