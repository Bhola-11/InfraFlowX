"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Permissions
...
logger = logging.getLogger("infraflowx.permissions.workflow")


class PermissionsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling permissions entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for permissions #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for permissions #{entity_id}: {curr} -> {target} by {user_role}")
