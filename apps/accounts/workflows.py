"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Accounts
...
logger = logging.getLogger("infraflowx.accounts.workflow")


class AccountsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling accounts entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for accounts #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for accounts #{entity_id}: {curr} -> {target} by {user_role}")
