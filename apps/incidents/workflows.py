"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Incidents
...
logger = logging.getLogger("infraflowx.incidents.workflow")


class IncidentsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling incidents entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for incidents #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for incidents #{entity_id}: {curr} -> {target} by {user_role}")
