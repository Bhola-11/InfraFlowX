"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Support
...
logger = logging.getLogger("infraflowx.support.workflow")


class SupportWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling support entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for support #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for support #{entity_id}: {curr} -> {target} by {user_role}")
