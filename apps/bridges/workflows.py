"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Bridges
...
logger = logging.getLogger("infraflowx.bridges.workflow")


class BridgesWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling bridges entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for bridges #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for bridges #{entity_id}: {curr} -> {target} by {user_role}")
