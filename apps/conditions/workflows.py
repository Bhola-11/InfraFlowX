"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Conditions
...
logger = logging.getLogger("infraflowx.conditions.workflow")


class ConditionsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling conditions entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for conditions #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for conditions #{entity_id}: {curr} -> {target} by {user_role}")
