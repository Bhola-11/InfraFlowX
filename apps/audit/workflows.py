"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Audit
...
logger = logging.getLogger("infraflowx.audit.workflow")


class AuditWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling audit entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for audit #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for audit #{entity_id}: {curr} -> {target} by {user_role}")
