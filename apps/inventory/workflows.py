"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Inventory
...
logger = logging.getLogger("infraflowx.inventory.workflow")


class InventoryWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling inventory entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for inventory #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for inventory #{entity_id}: {curr} -> {target} by {user_role}")
