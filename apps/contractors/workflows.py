"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Contractors
...
logger = logging.getLogger("infraflowx.contractors.workflow")


class ContractorsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling contractors entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for contractors #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for contractors #{entity_id}: {curr} -> {target} by {user_role}")
