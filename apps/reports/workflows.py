"""
InfraFlowX - Finite State Machine & Business Process Workflow Engine for Reports
...
logger = logging.getLogger("infraflowx.reports.workflow")


class ReportsWorkflowStateMachine:
    """
    Deterministic Finite State Machine (FSM) controlling reports entity lifecycles.
    """
    ...
    def execute_transition(
        ...
        if not cls.is_transition_permitted(curr, target):
            logger.warning(f"Invalid state transition attempted for reports #{entity_id}: {curr} -> {target}")
            return {
                "success": False,
        ...
        logger.info(f"State transition executed for reports #{entity_id}: {curr} -> {target} by {user_role}")
